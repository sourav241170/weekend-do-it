...
jQuery( window ).on( 'elementor/frontend/init', () => {
    ...
    elementorFrontend.hooks.addAction( 'frontend/element_ready/eafe-clients.default', addHandler );
    
    jQuery(window).load(function() {
        if(!jQuery('.owl-dots').hasClass('disabled')) {
            jQuery('.eafe-clients').addClass('owl-dots-enabled');
        } else {
            jQuery('.eafe-clients').removeClass('owl-dots-enabled');
        }
    });
} );