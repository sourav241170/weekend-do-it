...
class WP_Travel_Engine_Custom_Shortcodes {
	...
	*/
	...
	public static function wte_show_trip_map_shortcodes_callback( $attr ) {
		...
		if ( in_array( $attr['show'], array( 'both', 'iframe', 'iframe|image' ) ) && ! empty( $wp_travel_engine_setting['map']['iframe'] ) ) {
			?>
			<div class="trip-map iframe">
				<?php echo wptravelengine_esc_iframe( $wp_travel_engine_setting['map']['iframe'] ); ?>
			</div>
			<?php
	...
	function wte_trip_content( $atts ) {
		...
		if ( $query->have_posts() ) :
			...
			<div class="wte-category-outer-wrap">
			<?php
			$view_class = 'grid' === $atts['layout'] ? 'wte-col-3 category-grid' : 'category-list';
