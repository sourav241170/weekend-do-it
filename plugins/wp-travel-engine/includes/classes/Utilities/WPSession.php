 ...
 * @since   3.7.0
 */
final class WPSession extends RecursiveArrayAccess {
	/**
	 ...
	 * Singleton instance.
	 *
	 * @var WPSession|null $instance
	 */
	private static ?WPSession $instance = null;
	 ...
	 * @return $this
	 */
	public static function get_instance(): WPSession {
	...
	/**
	 * Default constructor.
	 * Will rebuild the session collection from the given session ID if it exists.
	 * Otherwise, we will
	...
	protected function __construct() {
		if ( isset( $_COOKIE[ WP_TRAVEL_ENGINE_SESSION_COOKIE ] ) ) {
			...
			$this->session_id = SessionUtils::generate_id();
			$this->set_expiration();
			$this->set_cookie();
		}
	 ...
	 * every 24 minutes.  After 30 minutes, the session will have expired. No cookie will be sent by
	 ...
	 * @return string
	 */
	public function json_out(): string {
	 ...
	 * @return bool
	 */
	public function json_in( string $data ): bool {
	 ...
	 *
	 * @param bool $delete_old Flag whether to delete the old session data from the server.
	 */
	public function regenerate_id( bool $delete_old = false ) {
		if ( $delete_old ) {
	 ...
	 * @return int
	 */
	public function cache_expiration(): int {
		return $this->expires;
	}
