...
<div class="wpte-bookings-contents">
	<?php
	foreach ( array_unique( $bookings ) as $key => $booking ) :
		...
		$today                  = date( 'Y-m-d' );
		$booked_date            = $booked_trip->datetime;
				...
				<div class="wpte-booked-trip-wrap">
					<div class="wpte-booked-trip-image">
						...
						if ( has_post_thumbnail( $booked_trip->ID ) ) {
							...
							?>
							<img alt="<?php the_title(); ?>" itemprop="image"
							...
							<div class="wpte-booked-trip-descriptions">
								<div class="wpte-booked-trip-inner-descriptions-left">
									<ul class="booking-status-info">
										<li>
											<span
												class="lrf-td-title"><?php esc_html_e( 'Departure:', 'wp-travel-engine' ); ?></span>
											<span
												class="lrf-td-desc"><?php echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( $booked_trip->datetime ) ) ); ?></span>
										</li>
										<li>
											<span
												class="lrf-td-title"><?php esc_html_e( 'Booking Status:', 'wp-travel-engine' ); ?></span>
											<span
												class="lrf-td-desc"><?php echo wp_kses( wptravelengine_booking_status( $booking_status ), array( 'code' => array() ) ); ?></span>
										</li>
									</ul>
								</div>
						...
						<div class="wpte-booked-trip-buttons-right">
							<?php if ( $show_pay_now_btn ) { ?>
								<a class="wpte-lrf-btn-transparent wpte-pay-btn"
								   ...
								   data-booking-id="<?php echo esc_attr( $booking ); ?>"
							...
							<?php } ?>
							<a class="wpte-lrf-btn-transparent wpte-detail-btn"
							   href="<?php echo esc_url( get_the_permalink() . '?action=booking-details&booking_id=' . $booking . '"' ); ?>"><?php esc_html_e( 'View Details', 'wp-travel-engine' ); ?></a>
						</div>
					</div>
				</div>
				<?php
