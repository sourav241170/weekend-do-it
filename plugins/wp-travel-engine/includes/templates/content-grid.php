...
echo $new_trip_listing ? ' wpte_new-layout' : '';
?>
" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
	<div class="category-trips-single-inner-wrap">
		<figure class="category-trip-fig">
			<?php
			if ( $new_trip_listing && $is_featured && $show_featured_tag ) :
				?>
				<div class="category-feat-ribbon">
					<span class="category-feat-ribbon-txt">
						<?php echo esc_html__( 'Featured', 'wp-travel-engine' ); ?>
					</span>
					<span class="cat-feat-shadow"></span>
				</div>
				<?php
			endif;
			if ( ( ! isset( $settings['display_new_trip_listing'] ) || $settings['display_new_trip_listing'] == 'no' ) && $is_featured ) :
				?>
				<div class="category-feat-ribbon">
					<span class="category-feat-ribbon-txt">
						<?php echo esc_html__( 'Featured', 'wp-travel-engine' ); ?>
			...
			if ( $group_discount ) :
				...
				<div class="category-trip-group-avil">
					...
					<span class="pop-trip-grpavil-txt">
						<?php echo esc_html( apply_filters( 'wp_travel_engine_group_discount_available_text', __( 'Group discount Available', 'wp-travel-engine' ) ) ); ?>
					</span>
				</div>
			<?php endif; ?>
		...
		<div class="category-trip-content-wrap">
			<div class="category-trip-prc-title-wrap">
				...
				if ( $show_wishlist ) {
					...
					<span class="wishlist-title">
						<?php __( 'Add to wishlist', 'wp-travel-engine' ); ?>
					</span>
			...
			<div class="category-trip-detail-wrap">
				<div class="category-trip-prc-wrap">
					<div class="category-trip-desti">
						...
						if ( ! empty( $pax ) ) :
							...
							<span class="category-trip-pax">
								...
								<span>
									<?php printf( esc_html__( '%s People', 'wp-travel-engine' ), esc_html( implode( '-', $pax ) ) ); ?>
								</span>
							</span>
						<?php endif; ?>
						<?php
						if ( $show_difficulty_tax ) :
							$taxonomy  = 'difficulty';
							$tax_terms = get_the_terms( $post->ID, 'difficulty' );
							if ( ! empty( $tax_terms ) ) {
								foreach ( $tax_terms as $term ) {
									...
									<span class="category-trip-difficulty">
										...
										printf(
											...
											esc_attr( $difficulty_span_class ),
											wp_kses_post( $difficulty_data_content ),
											esc_url( $difficulty_link ),
											esc_html( $difficulty_name ),
											wp_kses_post( $difficulty_levels )
										);
									?>
									</span>
									<?php
						...
						?>
						<?php
						if ( $new_trip_listing && $show_excerpt && $show_date_layout ) :
							?>
					...
					<?php if ( ! empty( $display_price ) || $show_date_layout ) : ?>
						<div class="category-trip-budget">
							<?php if ( $discount_percent ) : ?>
								<div class="category-disc-feat-wrap">
									<div class="category-trip-discount">
										<span class="discount-offer">
											<span><?php printf( esc_html__( '%1$s%% ', 'wp-travel-engine' ), esc_html( (float) $discount_percent ) ); ?></span>
											<?php esc_html_e( 'Off', 'wp-travel-engine' ); ?>
										</span>
									</div>
								</div>
								<?php
							...
							if ( $show_date_layout && false !== $fsds ) {
								if ( $fsds && is_array( $fsds ) ) {
									?>
									<div class="category-trip-dates">
										<span class="trip-dates-title">
											<?php echo esc_html__( 'Next Departure', 'wp-travel-engine' ); ?>
										</span>
										<?php
				...
				<?php
				if ( ( isset( $settings['display_new_trip_listing'] ) && 'yes' !== $settings['display_new_trip_listing'] ) || ! $show_date_layout ) :
					if ( $show_excerpt ) :
						?>
						<div class="category-trip-desc">
							<?php wptravelengine_the_trip_excerpt(); ?>
			...
			<?php
			$fsds            = apply_filters( 'trip_card_fixed_departure_dates', get_the_ID() );
			$new_date_layout = isset( $settings['display_new_trip_listing'] ) && ( ( $settings['display_new_trip_listing'] == 'yes' && $show_available_months ) || ( $settings['display_new_trip_listing'] == 'no' && ! $show_available_months ) );
			if ( ( ! isset( $settings['display_new_trip_listing'] ) || ( $new_date_layout ) ) ) :
				echo '<div class="category-trip-aval-time">';
				if ( false !== $fsds ) {
					if ( ( $fsds == get_the_ID() || empty( $fsds ) ) ) {
						?>
						<div class="category-trip-avl-tip-inner-wrap<?php echo ( ! $show_available_months ) ? '' : ' new-layout'; ?>">
							<span class="category-available-trip-text"><?php echo esc_html__( 'Available through out the year:', 'wp-travel-engine' ); ?></span>
							<?php if ( $show_available_months ) : ?>
							<i>
							...
							foreach ( range( 1, 12 ) as $month_number ) :
								echo '<li>' . esc_html( date_i18n( 'M', strtotime( "2021-{$month_number}-01" ) ) ) . '</li>';
							endforeach;
							echo '</ul></div>';
						...
						switch ( $dates_layout ) {
							case 'months_list':
								...
								if ( empty( $available_months ) ) {
									echo '<ul class="category-available-months">';
									foreach ( range( 1, 12 ) as $month_number ) :
										echo '<li>' . esc_html( date_i18n( 'n-M', strtotime( "2021-{$month_number}-01" ) ) ) . '</li>';
									endforeach;
									echo '</ul>';
									break;
								}
								...
								?>
									<div class="category-trip-avl-tip-inner-wrap<?php echo ( ! $show_available_months ) ? '' : ' new-layout'; ?>">
										<?php echo '<span class="category-available-trip-text"> ' . esc_html( $available_throughout ) . '</span>'; ?>
										<?php if ( $show_available_months ) : ?>
											<i>
												<svg width="24" height="24" viewBox="0 0 24 24" fill="none"
													xmlns="http://www.w3.org/2000/svg">
										...
										echo '<ul class="category-available-months">';
										foreach ( range( 1, 12 ) as $month_number ) {
											if ( isset( $available_months[ $month_number ] ) && $show_available_dates ) {
											...
											isset( $available_months[ $month_number ] ) ? printf( '<li class="' . esc_attr( $classname ) . '"' . wp_kses_post( $dates_attribute ) . '><a href="%1$s">%2$s</a></li>', esc_url( get_the_permalink() ) . '?month=' . esc_html( $available_months[ $month_number ] ) . '#wte-fixed-departure-dates', esc_attr( date_i18n( 'M', strtotime( "2021-{$month_number}-01" ) ) ) ) : printf( '<li><a href="#" class="disabled">%1$s</a></li>', esc_attr( date_i18n( 'M', strtotime( "2021-{$month_number}-01" ) ) ) );
										}
										echo '</ul>';
										?>
									</div>
							...
							case 'dates_list':
								...
								foreach ( $fsds as $fsd ) {
									...
									printf(
										...
										wte_esc_svg( $icon ),
										wte_esc_price( wte_get_formated_date( $fsd['start_date'] ) ),
										sprintf( esc_html__( '%s Available', 'wp-travel-engine' ), (float) $fsd['seats_left'] )
									);
								}
								echo '</ul>';
								echo '</div>';
