...
final class Plugin {
	...
	private function define_constants() {
		$constants = array(
			...
			'WP_TRAVEL_ENGINE_FILE_URL'            => plugin_dir_url( \WP_TRAVEL_ENGINE_FILE_PATH ),
	...
	private function load_dependencies() {
		...
		if ( ! defined( 'USE_WTE_LEGACY_VERSION' ) || ! USE_WTE_LEGACY_VERSION ) {
			...
			require_once sprintf( '%s/includes/helpers/helpers-analytics.php', WP_TRAVEL_ENGINE_ABSPATH );
		}
		 ...
		 * @since 5.9
		 */
		include_once sprintf( '%s/classes/Blocks/Metadata.php', WP_TRAVEL_ENGINE_ABSPATH );
		...
		if ( ! class_exists( 'CWPatternImport\CW_Pattern_Import' ) ) {
			include_once sprintf( '%sincludes/modules/pattern-inserter/class-import-patterns.php', WP_TRAVEL_ENGINE_ABSPATH );
		}
	}
	...
	private function define_admin_hooks() {
		 ...
		 * @since 5.5.3
		 */
		add_action( 'wptravelengine_booking_inventory', array(
	...
	/**
	 * Saves and updates customer data while creating cutomer.
	 *
	 * @param int $post_id
	 ...
	 * @since 5.7.2
	 */
	public function save_post_customer( $post_id, $post, $update ) {
		if ( ! $update ) {
