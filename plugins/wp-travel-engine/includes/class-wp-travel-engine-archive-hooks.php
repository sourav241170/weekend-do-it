...
class Wp_Travel_Engine_Archive_Hooks {
	...
	public static function get_featured_trips_ids() {
		...
		$prepared_statement = $wpdb->prepare( "SELECT `post_id` FROM {$wpdb->postmeta} WHERE `meta_key` = %s AND `meta_value` = %s LIMIT %d", 'wp_travel_engine_featured_trip', 'yes', $feat_trip_num );
	...
	public static function archive_filters_sub_options() {
		...
		<div class="wp-travel-toolbar clearfix">
			...
			<div class="wp-travel-engine-toolbar wte-view-modes">
				<?php
				$current_url    = '';
				$current_server = isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] === 'on' ? 'https://' : 'http://';

				if ( isset( $_SERVER['HTTP_HOST'] ) ) {
					$current_url .= esc_url_raw( wp_unslash( $current_server . $_SERVER['HTTP_HOST'] ) );
				}
			...
			if ( 'yes' === $collapsible_filter_panel ):
			?>
				<div class="wp-travel-engine-toolbar wte-filterbar-toggle">
					<button id="wte-filterbar-toggle-btn" class="wte-filterbar-toggle-btn">
						<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
							<mask id="mask0_79941_312" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="24" height="24">
								<rect width="24" height="24" fill="currentColor" />
							</mask>
							<g mask="url(#mask0_79941_312)">
								<path d="M20 4.42857C20 4.19142 19.7977 4 19.547 4H4.453C4.20233 4 4 4.19142 4 4.42857V6.41427H20V4.42857Z" fill="currentColor" />
								<path d="M18.3722 7.27136H4.27783L9.67158 11.9057V19.5714C9.67158 19.7199 9.75613 19.8599 9.89204 19.9371C9.96149 19.9799 10.043 19.9999 10.1246 19.9999C10.1971 19.9999 10.2695 19.9828 10.336 19.9485L14.0868 18.0856C14.2348 18.0114 14.3284 17.8656 14.3284 17.7056V11.9057L19.7222 7.27136H18.3722Z" fill="currentColor" />
							</g>
						</svg>
					</button>
				</div>
						...
						<div class="wte-category-outer-wrap">
							...
							while (have_posts()) :
								...
								if ( version_compare( '6.0.0', \WP_TRAVEL_ENGINE_VERSION, '<' ) ) {
									...
									wte_get_template( 'content-view.php', compact( 'details' , 'view_mode' ) );
								}
								$j++;
							endwhile;
							wp_reset_postdata();
