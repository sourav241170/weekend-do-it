...
<div class="wpte-block wpte-col3">
	...
	<div class="wpte-block-content">
		...
		if ( ! empty( $additional_fields ) ) {
			...
			?>
			<h4 class="wpte-title"><?php echo esc_html( $partial_label ) . ' ' . esc_html__( 'Payment Details', 'wp-travel-engine' ); ?></h4>
			<ul class="wpte-list">
				<?php
				...
				if ( isset( $booking_meta['place_order']['cost'] ) && ! empty( $booking_meta['place_order']['cost'] ) ) {
					?>
					<li>
						<b><?php echo esc_html( $partial_label . ' ' . esc_html__( 'Amount Paid', 'wp-travel-engine' ) ); ?></b>
						<span>
						<?php
						if ( isset( $booking_meta['place_order']['partial_cost'] ) && ! empty( $booking_meta['place_order']['partial_cost'] ) ) {
							echo wte_esc_price( wte_get_formated_price( $booking_meta['place_order']['partial_cost'] ) ); ////phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						} elseif ( isset( $booking_meta['place_order']['cost'] ) && ! empty( $booking_meta['place_order']['cost'] ) ) {
							echo wte_esc_price( wte_get_formated_price( $booking_meta['place_order']['cost'] ) ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
						?>
						</span>
					</li>
			...
			if ( isset( $booking_meta['place_order']['partial_due'] ) && ! empty( $booking_meta['place_order']['partial_due'] ) ) {
				?>
					<li>
						...
						<?php
						if ( isset( $booking_meta['place_order']['partial_due'] ) && ! empty( $booking_meta['place_order']['partial_due'] ) ) {
							echo wte_esc_price( wte_get_formated_price_html( $booking_meta['place_order']['partial_due'] ) ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
						?>
						</span>
					</li>
