			...
			<div style="margin-bottom: 40px;" class="wpte-info-block">
				...
				<p>
					<?php
						echo wp_kses(
							sprintf(
								__( 'Looking to personalize your booking and payment notifications? The Email Customizer add-on enables the creation of personalized email templates for booking and payment notifications, offering customization and flexibility. %1$sGet Email Customizer extension now%2$s.', 'wp-travel-engine' ),
								'<a target="_blank" href="https://wptravelengine.com/plugins/email-customizer/?utm_source=free_plugin&utm_medium=pro_addon&utm_campaign=upgrade_to_pro">',
								'</a>'
							),
							array(
								'a' => array(
...
if ( ! defined( 'WP_TRAVEL_ENGINE_PER_TRIP_EMAILS_VERSION' ) ) {
	?>
	<div class="wpte-form-block-wrap">
		<div class="wpte-form-block">
			<div style="margin-bottom: 40px;" class="wpte-info-block">
				...
				<p>
					<?php
						echo wp_kses(
							sprintf(
								__( 'Looking to optimize your website\'s booking process with personalized trip-specific emails? With Per Trip Email, customize multiple booking emails according to each trip, enhancing communication with your customers. %1$sGet Per Trip Email extension now%2$s.', 'wp-travel-engine' ),
								'<a target="_blank" href="https://wptravelengine.com/plugins/per-trip-emails/?utm_source=free_plugin&utm_medium=pro_addon&utm_campaign=upgrade_to_pro">',
								'</a>'
							),
							array(
								'a' => array(
...
<div class="wpte-field wpte-checkbox advance-checkbox">
	...
	<span
		class="wpte-tooltip"><?php echo esc_html( 'Turn this off if you do not want to send email notification to customer.', 'wp-travel-engine' ); ?></span>
</div>
...
<div class="<?php wptravelengine_hidden_class( 'no' === $enable_customer_notification, true ); ?>"
	...
	<div class="wpte-field wpte-textarea wpte-floated">
		...
		<div class="wpte-field wpte-textarea wpte-floated wpte-rich-textarea delay">
				<textarea
					...
					class="wte-editor-area"
					id="<?php echo esc_attr( $editor_id ); ?>"><?php echo wp_kses_post( $value_wysiwyg ); ?></textarea>
		</div>
		<span
			class="wpte-tooltip"><?php esc_html_e( 'This template will be used when a booking request has been made by the customer.', 'wp-travel-engine' ); ?> <a
				class="wte-email-template-preview-link"
				href="<?php echo esc_url( home_url( '/' ) ); ?>?_action=email-template-preview&template_type=order&pid=0&to=customer"><?php esc_html_e( 'Preview Template', 'wp-travel-engine' ); ?></a></span>
	...
	<div class="wpte-field wpte-textarea wpte-floated">
		...
		<div class="wpte-field wpte-textarea wpte-floated wpte-rich-textarea delay">
				<textarea
					placeholder="<?php esc_attr_e( 'Email Message', 'wp-travel-engine' ); ?>"
					name="wp_travel_engine_settings[email][<?php echo esc_attr( $editor_id ); ?>]"
					class="wte-editor-area wp-editor-area"
					id="<?php echo esc_attr( $editor_id ); ?>"><?php echo wp_kses_post( $value_wysiwyg ); ?></textarea>
		</div>
		<span
			class="wpte-tooltip"><?php esc_html_e( 'This email template will be used when ever a payment received.', 'wp-travel-engine' ); ?><a
				class="wte-email-template-preview-link"
				href="<?php echo esc_url( home_url( '/' ) ); ?>?_action=email-template-preview&template_type=order_confirmation&pid=0&to=customer"><?php esc_html_e( 'Preview Template', 'wp-travel-engine' ); ?></a></span>
	...
	<div class="wpte-field wpte-tags">
		...
		<ul class="wpte-list">
			...
			<li>
				<b>{payment_method}</b>
				<span><?php esc_html_e( 'Payment Method used to checkout.', 'wp-travel-engine' ); ?></span>
			</li>
		</ul>
		<?php
		/**
