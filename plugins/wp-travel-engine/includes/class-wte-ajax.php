...
class WTE_Ajax {
	...
	public static function add_to_cart() {
		...
		// Decoded JSON data.
		$cart_data = json_decode( $raw_data, true );
		...
		$attrs      = apply_filters(
			...
			array(
				'trip_date'          => $cart_data->{'tripDate'},
				'trip_time'          => $cart_data->{'tripTime'},
				'trip_time_range'    => isset( $cart_data->{'timeRange'} ) ? $cart_data->{'timeRange'} : array(),
	...
	** Update user wishlist
	...
	public static function ajax_request_middleware() {
		...
		$action = str_replace( '-', '_', $request->get_param( 'action' ) );

		$use_middleware = apply_filters( "wp_travel_engine_ajax_action_$action", true );
		if ( method_exists( static::class, "action_{$action}" ) && $use_middleware ) {
			static::{"action_{$action}"}();
		}
	}

	protected static function action_wte_payment_stripe_charge() {
		global $wte_cart;
		...
		switch ( $payment_type ) {
			case 'full':
				$amount = $wte_cart->get_cart_total();
				break;
			case 'partial':
				$amount = $wte_cart->get_total_partial();
				break;
			case 'due':
				/** @var Booking $booking */
				$booking = Booking::make( $wte_cart->get_booking_ref() );
				$amount  = $booking->get_due_amount();
				break;
		}
