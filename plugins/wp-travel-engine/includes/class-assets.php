...
class Assets {
	...
	/**
	 * Retrieves the localize data for the payment process.
	 *
	...
	public function get_payment_localize_data(): array {
		...
		$data = array(
			'locale'        => get_locale(),
			'total'         => $wte_cart->get_totals(),
			'total_partial' => $wte_cart->get_total_partial(),
		);

		if ( ( $_GET[ 'action' ] ?? false ) === 'partial-payment' && ! empty( $_GET[ 'booking_id' ] ) ) {
			$booking                 = Core\Booking::get_booking_info_by_id( $_GET[ 'booking_id' ] );
			$data[ 'total' ]         = round( $booking->cart_info[ 'total' ], 2 );
			$data[ 'total_partial' ] = round( $booking->cart_info[ 'cart_partial' ], 2 );
		}
