	...
	<div class="category-trips-single-inner-wrap">
		...
		<div class="category-trip-content-wrap">
			<div class="category-trip-prc-title-wrap">
				<?php
				if ( wte_array_get( $settings, 'layoutFilters.showTitle', true ) ) :
				$taglevel = wte_array_get( $settings, 'tripTitleHeadingLevel', 2 );
				?>
					<h<?php echo esc_attr( $taglevel ); ?> class="category-trip-title" itemprop="name">
						<a itemprop="url" href="<?php echo esc_url( get_the_permalink( $trip ) ); ?>"><?php echo esc_html( $trip->post_title ); ?></a>
					</h<?php echo esc_attr( $taglevel ); ?>>
				<?php endif; ?>

				<?php
				if ( wte_array_get( $settings, 'layoutFilters.showReviews', false ) ) :
					echo \wte_get_the_trip_reviews( $trip->ID ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				endif;
				?>
					...
					<div class="category-trip-desti">
						...
						<?php if ( wte_array_get( $settings, 'layoutFilters.showGroupSize', false ) && (int) $meta->min_pax ) : ?>
							<span class="wpte-trip-meta category-trip-dur wpte-trip-pax">
								...
								<?php printf( '<span>' . esc_html__( '%s People', 'wp-travel-engine' ) . '</span>', (int) $meta->max_pax ? esc_html( $meta->min_pax . '-' . $meta->max_pax ) : esc_html( $meta->min_pax ) ); ?>
							</span>
						<?php endif; ?>
					</div>
		...
		if ( $show_available_months || wte_array_get( $settings, 'layoutFilters.showViewMoreButton', true ) ) {
			...
			if ( $show_available_months ) {
				...
				if ( $fsds == $trip->ID ) {
					...
					echo '<span class="category-available-trip-text"> ' . esc_html__( 'Available through out the year:', 'wp-travel-engine' ) . '</span>';
					...
					switch ( $dates_layout ) {
						case 'months_list':
							...
							echo '<ul class="category-available-months">';
							foreach ( range( 1, 12 ) as $month_number ) {
								isset( $available_months[ $month_number ] ) ? printf( '<li><a href="%1$s">%2$s</a></li>', esc_url( get_the_permalink() ) . '?month=' . esc_html( $available_months[ $month_number ] ) . '#wte-fixed-departure-dates', esc_html( date_i18n( 'M', strtotime( "2021-{$month_number}-01" ) ) ) ) : printf( '<li><a href="#" class="disabled">%1$s</a></li>', esc_html( date_i18n( 'M', strtotime( "2021-{$month_number}-01" ) ) ) );
							}
							echo '</ul>';
							echo '</div>';
							break;
						case 'dates_list':
							...
							foreach ( $fsds as $fsd ) {
								...
								printf(
									...
									wte_esc_svg( $icon ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
