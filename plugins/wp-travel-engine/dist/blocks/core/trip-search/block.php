...
if(  is_array( $search_filters )){
	...
	$search_filters,
	function( $sf1, $sf2 ) {
		if( isset( $sf1['order'] ) && isset( $sf2['order'] ) )
						...
						<div class="wpte-trip__adv-field wpte__select-field">
							<span class="icon">
								...
								if ( isset( $icons[ $_taxonomy ] ) ) {
									echo wte_esc_svg( $icons[ $_taxonomy ] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								} else {
									echo wte_esc_svg( $icons['default'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								}
								?>
							</span>
