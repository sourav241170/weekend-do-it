	...
	<div class="wpte-trip-sfilter-wrapper">
		<form class="wpte-trip__search-fields<?php $is_horizontal && print( ' horizontal' ); ?>" action="<?php echo esc_url( wp_travel_engine_get_page_permalink( 'search' ) ); ?>">
				...
				if ( $_args['show'] ) {
					...
					if ( is_array( $terms ) && count( $terms ) > 0 ) {
						...
						<div class="wpte-trip__adv-field wpte__select-field">
							<span class="icon">
								<?php
								if ( isset( $_args['icon']['value'] ) && $_args['icon']['value'] != '' ) {
									\Elementor\Icons_Manager::render_icon( $_args['icon'] );
								} elseif ( isset( $_args['icon'] ) && '' === $_args['icon']['value'] && isset( $icons[ $_taxonomy ] ) ) {
			...
			<div class="wpte-trip__submit-field">
				<button type="submit" class="wpte-trip__search-submit"><?php echo esc_html__( $attributes->{'searchButtonLabel'}, 'wptravelengine-elementor-widgets' ); ?></button>
			</div>
		</form>
	</div>
</div>
